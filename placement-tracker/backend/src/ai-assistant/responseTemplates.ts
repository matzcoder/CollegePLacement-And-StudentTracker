...
export function buildResponse(intent: string, data: Record<string, unknown>): string {
  switch (intent) {
    ...
    case 'where_applied': {
      ...
      const list = apps.map((a) => `• ${a.company} — Stage: ${a.stage}`).join('\n');
      return `You have applied to the following companies:\n${list}`;
    }

    case 'shortlist_status': {
      const shortlisted = (data.applications as AppEntry[] | undefined)?.filter(
    ...
    case 'offer_status': {
      const offers = (data.applications as AppEntry[] | undefined)?.filter(
      ...
      return `Offer details:\n${offers
        .map((a) => `• ${a.company}: ${a.offerStatus}${a.package ? ` — ₹${(a.package / 100000).toFixed(1)}L CTC` : ''}`)
        .join('\n')}`;
    }

    case 'package_query': {
      const pkgApps = (data.applications as AppEntry[] | undefined)?.filter(
    ...
    case 'application_status': {
      ...
      return `Your application pipeline:\n${apps
        .map((a) => `• ${a.company}: ${a.stage} / ${a.offerStatus}`)
        .join('\n')}`;
    }
